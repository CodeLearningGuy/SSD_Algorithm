...
def up_down_decom(d_MP, d_MC, Pr, num, cap):
    ...
    for i in range(n_d_MC):
        ...
        while k:
            ...
            # print(i)
            # print(qlfd_d_MP[sel_H_min_ind, :])
            # print(v0)
            # print(b0)
            # # print(np.array_equal(v0, qlfd_d_MP[sel_H_min_ind, :]))

            # 每个空间分解得到的下界不一定都是d-MP,所以应该记录v0
            # 将每个新得到的v0作为键
            key = tuple(v0)
            # 检查v0_up_matrix中是否已经存在该键
            if key in v0_up_matrix:
                # 如果键存在，则在对应分解得到的上界矩阵后面添加新的上界b0
                v0_up_matrix[key] = np.vstack([v0_up_matrix[key], b0])
                # v0_up_matrix[key][0] = np.vstack([v0_up_matrix[key][0], b0])
                # # 构建二维索引第一维i以减少空间相交条件判断次数（行不通）
                # v0_up_matrix[key][1].append([i, None])
            else:
                # 如果键不存在，则创建一个新的矩阵，并将b0作为第一行
                v0_up_matrix[key] = b0
                # v0_up_matrix[key] = [b0, [[i, None]]]
            num_space += 1
    ...
    # 再利用得到的上界分解每个v0
    # 构建二维索引第二维j以减少空间相交条件判断次数
    # for i in range(len(v0_up_matrix)):
    for key, up_vec in v0_up_matrix.items():
        # 将元组键转化为 NumPy 行向量
        b0 = np.array(key)
        ...
        # 键值对的值对应的是二维列表，第一维是up_vec，第二维是相交空间判断所需的二维索引
        if len(up_vec.shape) <= 1:
            continue
    ...
    # 空间预处理结束后生成的空间数
    space_num = len(down_matrix)
    # 相交空间再分解的子空间数sub_spc_num
    sub_spc_num = 1
    # 按顺序遍历空间，存储与每个空间相交的所有空间（深度优先分解）
    for i in range(space_num):
        spc_low_bound = down_matrix[i, :]
        spc_up_bound = up_matrix[i, :]
        for j in range(i+1, space_num):
            space_id = space_num
...
if __name__ == '__main__':
    ...
    d_mp = np.array([[2, 1, 1, 2, 1], [1, 2, 1, 1, 2], [2, 1, 1, 1, 3]])
    d_mc = np.array([[3, 2, 2, 2, 3], [3, 3, 2, 3, 3], [3, 3, 3, 2, 2]])
